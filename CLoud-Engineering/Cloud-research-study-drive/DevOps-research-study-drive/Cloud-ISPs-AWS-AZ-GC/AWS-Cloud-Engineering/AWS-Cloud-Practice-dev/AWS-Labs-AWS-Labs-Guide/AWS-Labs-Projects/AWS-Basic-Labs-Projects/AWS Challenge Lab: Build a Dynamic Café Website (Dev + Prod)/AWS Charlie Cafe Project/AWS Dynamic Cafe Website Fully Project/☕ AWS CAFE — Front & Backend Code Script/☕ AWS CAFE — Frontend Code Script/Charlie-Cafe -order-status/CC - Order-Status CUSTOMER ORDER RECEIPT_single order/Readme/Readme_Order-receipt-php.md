...
---
### Latest Fully Final order-receipt.php
...
Absolutely! You can use config.js even in PHP pages, because PHP runs on the server but config.js runs in the browser (client-side).

The idea:

- Keep your API base URL dynamic by reading it from config.js.

- Use a small JS snippet to fetch the API endpoint dynamically instead of hardcoding it in PHP.

- For a fully production-ready page, you can combine PHP for initial order validation + JS for fetching the order from API dynamically.

### Here’s the fully refactored order-receipt.php:
...
API Base URL is dynamic:

```
const apiBase = `${window.CHARLIE_CONFIG.API_BASE}/order-status`;
```

- No need to edit PHP if your API endpoint changes in the future. Just update config.js.

- Order details are filled dynamically via JS:

- PHP only validates order_id server-side.

- JS fetches the latest order details live from the API.

- Live refresh every 10 seconds for order status updates.

- QR Code is generated dynamically.

- Fully clean, modular, and maintainable.
