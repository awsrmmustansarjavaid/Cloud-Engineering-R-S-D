...
Proper orders fetching using CHARLIE_API.getOrders()

KPI cards calculation based on the orders
...
- Replaced adminDashboard.fetchData() with CHARLIE_API.getOrders().

- Orders table now shows: Customer, Item, Qty, Table, Total, Status, Payment.

- KPI cards (Sales, Orders, Drinks, Avg) are calculated dynamically.

- Page auto-refreshes orders every 30 seconds.

- HR section unchanged and works with your analytics API.

- Printing still works via openCentralPrint().

- Full comments added for clarity.
